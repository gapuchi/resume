...
\begin{itemize}[leftmargin=0.15in, label={}]
    ...
    \item\begin{tabular*}{0.97\textwidth}[t]{l@{\extracolsep{\fill}}r}\textit{\small{ {% if area.studyType == "Bachelor of Science" %} B.S {{area.area}} (GPA: {{area.score}}) {% else %} Minor {{area.area}} {% endif %} }} &\textit{\small{ {% if loop.first %} {{area.startDate | date(format="%b %Y")}} -- {{area.endDate | date(format="%b %Y")}} {% endif %} }}\\\end{tabular*}
    {%- endfor %}
