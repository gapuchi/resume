...
\begin{itemize}[leftmargin=0in, label={}, itemsep=0.35em]

    \item\begin{minipage}[t]{0.97\textwidth}
        \begin{tabular*}{\textwidth}[t]{l@{\extracolsep{\fill}}r}
            \textbf{Traba} & New York, NY\\
            \textit{\small{Senior Software Engineer}} & \textit{\small{Jul 2024 -- Oct 2025}}\\
        \end{tabular*}
        \begin{itemize}
            \item\small{Automated \textasciitilde60\% of our Operations team's actions to fill shifts by architecting a scalable, extensible automation platform. Enabled devs to implement any programmable action and orchestrate them in a series of steps. The platform was later used to automattically onboard and prep workers for their first shifts.}
            \item\small{Developed the integration framework for our backend services, eliminating the need for daily QA checks before deploying to production. This saved 20+ engineering hours per week, enabled us to build out our CD pipeline which led to faster and more frequent deployments to production.}
            \item\small{Improved dev experience by reducing build times from minutes to \textasciitilde30 seconds, introducing structured logging + metrics, optimizing deployments by eliminating redundant docker image builds, and fixing memory leaks in our local deployments.}
        \end{itemize}
    \end{minipage}

    \item\begin{minipage}[t]{0.97\textwidth}
        \begin{tabular*}{\textwidth}[t]{l@{\extracolsep{\fill}}r}
            \textbf{ModernFi} & New York, NY\\
            \textit{\small{Senior Software Engineer, Tech Lead}} & \textit{\small{Nov 2023 -- May 2024}}\\
        \end{tabular*}
        \begin{itemize}
    ...
    \item\begin{minipage}[t]{0.97\textwidth}
        \begin{tabular*}{\textwidth}[t]{l@{\extracolsep{\fill}}r}
            \textbf{Amazon Elastic Map Reduce (EMR) Serverless (Amazon Web Services)} & Seattle, WA\\
            \textit{\small{Founding Engineer, Software Development Engineer II, Tech Lead}} & \textit{\small{Apr 2021 -- Nov 2023}}\\
        \end{tabular*}
        \begin{itemize}
            ...
            \item\small{Collaborated with downstream services to overcome capacity constraints brought on by heavy users, those who consumed at least 4,000 vCPU's concurrently, of EMR Serverless.}
    ...
    \item\begin{minipage}[t]{0.97\textwidth}
        \begin{tabular*}{\textwidth}[t]{l@{\extracolsep{\fill}}r}
            \textbf{Universal Brand Catalog (Amazon.com)} & Seattle, WA\\
            \textit{\small{Software Development Engineer}} & \textit{\small{Aug 2017 -- Apr 2021}}\\
        \end{tabular*}
        \begin{itemize}
...
\begin{tabular*}{\textwidth}{p{0.12\linewidth}p{0.88\linewidth}}
    \small{\textbf{Languages}} & \small{GoLang | Java | TypeScript | Python | Scala | SQL | Rust | C/C++}\\
    \small{\textbf{AWS}} & \small{ECS | ECR | Fargate | Lambda | Step Function | DynamoDB | CloudFormation | CloudWatch | S3 | CDK | IAM | KMS | VPC}\\
    \small{\textbf{Tooling}} & \small{Postgres | Docker + containerd | Apache Spark | Spring | Guice | React | Smithy | CI/CD}\\
\end{tabular*}
